...
namespace ShyUI
{
    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Shy UI failed to start:\n\n" + ex.Message, "Shy UI",
    ...
    public class ShyUIContext : ApplicationContext
    {
        ...
        private Dictionary<IntPtr, bool> windowWasMaximized;
        ...
        public ShyUIContext()
        {
            ...
            windowWasMaximized = new Dictionary<IntPtr, bool>();
            ...
            loopTimer = new Timer();
            loopTimer.Interval = 50; // 20fps is plenty for window management
        ...
        public void TogglePause()
        {
            ...
            if (isSystemPaused)
            {
                // Restore managed windows to their pre-managed state
                foreach (IntPtr handle in windowRevealedState.Keys.ToList())
                {
                    bool wasMaximized;
                    if (windowWasMaximized.TryGetValue(handle, out wasMaximized) && wasMaximized)
                    {
                        Win32.ShowWindow(handle, Win32.SW_SHOWMAXIMIZED);
                    }
                    // Windows that were already normal stay where they are.
                }
                windowRevealedState.Clear();
                windowWasMaximized.Clear();
        ...
        public void ToggleCurrentApp()
        {
            ...
            if (autoManagedWindows.Contains(hWnd))
            {
                ...
                windowWasMaximized.Remove(hWnd);
                windowRevealedState.Remove(hWnd);
                Win32.ShowWindow(hWnd, Win32.SW_SHOWMAXIMIZED);
                ShowBalloon("Shy UI", string.Format("Removed window from Auto-Shy UI"));
            ...
            if (appConfigs.ContainsKey(procName))
            {
                ...
                windowRevealedState.Remove(hWnd);
                Win32.ShowWindow(hWnd, Win32.SW_SHOWMAXIMIZED);
                ShowBalloon("Shy UI", string.Format("Removed {0} from Shy UI", procName));
                Logger.Log("Removed app: " + procName);
            }
            else
            {
                appConfigs[procName] = defaultTopBarHeight;
                ShowBalloon("Shy UI", string.Format("Added {0} to Shy UI", procName));
        ...
        private void PruneDeadWindows()
        {
            ...
            foreach (IntPtr handle in autoManagedWindows)
            {
                if (!Win32.IsWindow(handle)) dead.Add(handle);
            }
            foreach (IntPtr handle in dead)
            {
                autoManagedWindows.Remove(handle);
                windowWasMaximized.Remove(handle);
                windowRevealedState.Remove(handle);
            }
            var stale = new List<IntPtr>();
            foreach (IntPtr handle in windowRevealedState.Keys)
            {
                if (!Win32.IsWindow(handle)) stale.Add(handle);
            }
        ...
        private void ManageWindow(IntPtr hWnd, int topBarHeight, string procName)
        {
            ...
            // Restore if maximized (only once; afterwards the window is normal-sized)
            if (IsMaximized(hWnd))
            {
